...
netfile = 'net.weights.h5'  # 构建的神经网络模型存储路径，文件名以 .weights.h5 结尾
...
predict_result = net.predict(train[:,:3]).reshape(len(train))  # 预测结果变形
'''这里要提醒的是，keras用predict给出预测概率，predict_classes才是给出预测类别，而且两者的预测结果都是n x 1维数组，而不是通常的 1 x n'''

from cm_plot import *  # 导入自行编写的混淆矩阵可视化函数
cm_plot(train[:,3], (predict_result > 0.5).astype(int)).show()  # 显示混淆矩阵可视化结果
